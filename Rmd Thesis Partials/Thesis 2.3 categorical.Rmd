...
pheatmap(coef_nz, 
         cluster_rows = TRUE, 
         cluster_cols = FALSE, 
         main = paste0("Non-zero Coefficients at λ_min = ", round(cv_lasso$lambda.min, 4)),
...
```{r}
# Create PCA for 2D visualization using ALL data (not just PCs used in model)
# This ensures we can project back and forth
pca_viz <- prcomp(df_scaled[, -ncol(df_scaled)], center = TRUE, scale. = TRUE)

# Project training data to first 2 PCs
train_pca_2d <- data.frame(
  PC1 = pca_viz$x[train_index, 1],
  PC2 = pca_viz$x[train_index, 2],
  response = df_scaled$response[train_index]
)

# Create grid in PC space
x_range <- range(train_pca_2d$PC1) + c(-0.5, 0.5) * diff(range(train_pca_2d$PC1))
y_range <- range(train_pca_2d$PC2) + c(-0.5, 0.5) * diff(range(train_pca_2d$PC2))
grid <- expand.grid(
  PC1 = seq(x_range[1], x_range[2], length.out = 100),
  PC2 = seq(y_range[1], y_range[2], length.out = 100)
)

# Project grid points back to original feature space
# Inverse PCA: grid_orig = grid * rotation^T + center
grid_matrix <- as.matrix(grid)
rotation_2d <- pca_viz$rotation[, 1:2]
grid_orig <- grid_matrix %*% t(rotation_2d)

# Add back the scaling and centering
if (!is.null(pca_viz$center)) {
  grid_orig <- sweep(grid_orig, 2, pca_viz$center, "+")
}

# Convert to data frame with original column names
grid_orig_df <- as.data.frame(grid_orig)
colnames(grid_orig_df) <- names(pca_viz$center)

# Now predict using the SVM model (which expects original feature space)
# The svm_linear model from caret can predict on new data directly
grid_preds <- predict(svm_linear, newdata = grid_orig_df)

# Add predictions to grid
grid$Class <- grid_preds

# Calculate variance explained
var_pc1 <- round(summary(pca_viz)$importance[2, 1] * 100, 1)
var_pc2 <- round(summary(pca_viz)$importance[2, 2] * 100, 1)

# Create the decision boundary plot
ggplot() +
  # Decision regions
  geom_tile(data = grid, aes(x = PC1, y = PC2, fill = Class), alpha = 0.3) +
  # Training data points
  geom_point(data = train_pca_2d, aes(x = PC1, y = PC2, color = response), 
             size = 2, alpha = 0.7) +
  # Confidence ellipses for training data
  stat_ellipse(data = train_pca_2d, aes(x = PC1, y = PC2, color = response), 
               level = 0.95, alpha = 0.5, linewidth = 0.8) +
  theme_minimal() +
  labs(title = "SVM Linear - Decision Boundaries (First 2 PCs)",
       subtitle = paste0("Cost C = ", svm_linear$bestTune$cost, 
                         " | CV Accuracy = ", round(max(svm_linear$results$Accuracy), 4),
                         " | Test Accuracy = ", round(test_acc_linear, 4)),
       x = paste0("PC1 (", var_pc1, "%)"),
       y = paste0("PC2 (", var_pc2, "%)"),
       color = "Actual Class",
...
rf_model <- train(response ~ ., data = train_data,
                  ...
                  trControl = trainControl(method = "cv", number = 10,
                                           summaryFunction = multiClassSummary),
                  tuneGrid = data.frame(mtry = results[["ntree_250"]]$bestTune$mtry),
                  ntree = 250)
preds_rf <- predict(rf_model, newdata = test_data)
cm_rf <- confusionMatrix(preds_rf, test_data$response, mode = "prec_recall")
...
# Train a single decision tree for visualization (not the random forest)
...
rpart.plot(tree_model,
           ...
           extra = 101,        # Show predicted class and probabilities
           fallen.leaves = TRUE,
           main = "Decision Tree (for visualization, not the RF model)",
           box.palette = "RdYlGn",
           shadow.col = "gray",
